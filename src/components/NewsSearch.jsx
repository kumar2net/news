    ...
    const fetchTopics = async () => {
      ...
      try {
        ...
        const merged = results
          ...
          .filter(
            (article, index, self) =>
              self.findIndex((candidate) => candidate.url === article.url) === index,
          )
          .sort(
    ...
    <section className="space-y-8">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-3xl border border-white/70 bg-white p-6 shadow-[0_10px_40px_rgba(94,53,177,0.08)]">
          <h3 className="text-xl font-semibold text-slate-900">Craft your search</h3>
          <p className="mt-1 text-sm text-slate-500">
            Separate topics with commas. We’ll fetch five fresh stories per topic and blend them
            together.
          </p>
        ...
        <aside className="rounded-3xl border border-white/70 bg-gradient-to-br from-white via-[#ede7f6] to-white p-6 shadow-[0_10px_40px_rgba(124,77,255,0.12)]">
          <h3 className="text-base font-semibold text-[#5e35b1]">Tips for richer results</h3>
      ...
      <section className="space-y-4">
        ...
        {status === 'success' && articles.length === 0 && (
          <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center text-slate-500">
            No articles found for the requested topics. Try adjusting keywords or include broader
            synonyms.
          </div>
        )}
                ...
                >
                  ...
                  <div className="mt-6 flex items-center justify-between text-sm">
                    <span className="text-slate-500">
                      {article.author ? `By ${article.author}` : 'Curated for you'}
                    </span>
                    <a
                      href={article.url}
                      target="_blank"
